...
namespace DatingApp.API.Data
{
    public class DatingRepository: IDatingRepository
    {
        ...
        {
            var users= await _context.Users.Include(c => c.Liker).Include(x=> x.Likee).FirstOrDefaultAsync(u=> u.Id == id);
            if (likers)
            {
                return users.Liker.Where(u => u.LikeeId == id).Select(i => i.LikerId);
            }
            else {
                return users.Likee.Where(u => u.LikerId == id).Select(i => i.LikeeId);
            }
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientId)
        {
            ...
            .ThenInclude(p => p.Photos).Include(u => u.Recipient)
            .ThenInclude(p => p.Photos).Where(m => m.RecipientId ==userId && m.RecipientDeleted == false && m.SenderId == recipientId
